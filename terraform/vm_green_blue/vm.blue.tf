resource "azurerm_network_interface" "blue" {
  ...
  name                = "nic-${local.base_name}-blue${count.index}"
...
resource "azurerm_linux_virtual_machine" "blue" {
  count                           = var.blue_count
  name                            = "vm-${local.base_name}-blue${count.index}"
  location                        = azurerm_resource_group.main.location
  resource_group_name             = azurerm_resource_group.main.name
  size                            = "Standard_B1s"
...
resource "azurerm_network_interface_backend_address_pool_association" "blue" {
  ...
  ip_configuration_name   = azurerm_network_interface.blue[count.index].ip_configuration[0].name
