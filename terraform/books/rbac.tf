...
resource "azurerm_role_assignment" "storage_function_books" {
  scope                = azurerm_storage_account.main.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_linux_function_app.books.identity[0].principal_id
}

resource "azurerm_role_assignment" "storage_function_versionme" {
  scope                = azurerm_storage_account.main.id
  role_definition_name = "Storage Blob Data Contributor"
  principal_id         = azurerm_linux_function_app.versionme.identity[0].principal_id
}
